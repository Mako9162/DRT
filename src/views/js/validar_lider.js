...
$(document).ready(function () {
    ...
    $("#env_val").on("click", function () {
      ...
      if(rows_selected.length === 0){
        swal("Error", "Debe seleccionar al menos una fila antes de enviar las tareas al Lider Técnico.", "error");
        return;
      }
    ...
    $('#enviarCodigo2').on('click', function() {
        ...
        $.each(rows_selected, function (index, row) {
            ...
            var inputValue = $(row).find('td').eq(11).find('input').val();
            let boolobs;
